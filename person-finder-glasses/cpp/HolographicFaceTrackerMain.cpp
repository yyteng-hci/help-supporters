...
void HolographicFaceTrackerMain::ProcessFaces(std::vector<BitmapBounds> const& faces, Windows::Media::Capture::Frames::MediaFrameReference^ frame, SpatialCoordinateSystem^ worldCoordSystem)
{
    ...
    for (BitmapBounds const& faceRect : faces)
    {
        ...
        if (dotFaceWithGaze > bestDotProduct)
        {
            ...
            if (bestRectPositionInCameraSpace.z < 0)
            {
                OutputDebugStringA(" ***NEGATIVE TARGET*** ");
                OutputDebugStringA(std::to_string(bestRectPositionInCameraSpace.z).c_str());
                OutputDebugStringA(" ***NEGATIVE TARGET*** ");
                bestRectPositionInCameraSpace = {std::abs(bestRectPositionInCameraSpace.x),std::abs(bestRectPositionInCameraSpace.y),std::abs(bestRectPositionInCameraSpace.z)};
            }
        }
    }

    // Transform the cube from Camera space to World space.
    float3 bestRectPositionInWorldspace = transform(bestRectPositionInCameraSpace, cameraToWorld->Value);
    
    bestRectPositionInWorldspace = {std::abs(bestRectPositionInWorldspace.x),std::abs(bestRectPositionInWorldspace.y),std::abs(bestRectPositionInWorldspace.z)};
    Windows::Foundation::Numerics::float3 target = bestRectPositionInWorldspace + cubeOffsetInWorldSpace;
   
    target = {std::abs(target.x),std::abs(target.y),std::abs(target.z)};
    m_spinningCubeRenderer->SetTargetPosition(target);
